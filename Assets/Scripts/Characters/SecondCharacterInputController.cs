...
/// </summary>
public class SecondCharacterInputController : InputController
{
	...
	protected void Update ()
    {
        ...
        else if(Input.GetKeyDown(KeyCode.W) && isVerticalMovementEnabled)
        {
            Jump();
        }
		else if (Input.GetKeyDown(KeyCode.S) && isVerticalMovementEnabled)
		{
			if(!m_Sliding)
				Slide();
		}
		...
		if (m_Sliding)
		{
			...
			float correctSlideLength = slideLength * (1.0f + trackManager.humanCharSpeedRatio);
        ...
        if(m_Jumping)
        {
			if (trackManager.isHumanCharMoving)
			{
				...
				float correctJumpLength = jumpLength * (1.0f + trackManager.humanCharSpeedRatio);
    ...
    public void Jump()
    {
        if (!m_Jumping)
        {
            ...
            float animSpeed = k_TrackSpeedToJumpAnimSpeedRatio * (trackManager.humanCharSpeed / correctJumpLength);
	...
	public void Slide()
	{
		if (!m_Sliding && !m_Jumping)
		{
            ...
            float animSpeed = k_TrackSpeedToJumpAnimSpeedRatio * (trackManager.humanCharSpeed / correctSlideLength);
